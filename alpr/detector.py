...
class PlateDetector:
    ...
    def resize_rect(self,x1,y1,x2,y2,factor_ancho=1,factor_alto=1):

        ancho       = int(x2 - x1)
        new_ancho   = ancho*factor_ancho
        dif_ancho   = new_ancho - ancho
        res_ancho   = int(dif_ancho/2)

        alto        = int(y2 - y1)
        new_alto    = alto*factor_alto
        dif_alto    = new_alto - alto
        res_alto   = int(dif_alto/2)

        new_x1,new_x2 = int(x1- res_ancho),int(x2+ res_ancho)
        new_y1,new_y2 = int(y1+ res_alto),int(y2- res_alto)
    ...
    def yield_coords(self, frame: np.ndarray, bboxes: list):
        ...
        for i in range(num_boxes[0]):
            ...
            x1, y1, x2, y2 = self.resize_rect(x1, y1, x2, y2,0.9, 0.9)
