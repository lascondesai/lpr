...
class PlateOCR:
    ...
    def __init__(self,ocr_model_num: int = 4,confianza_avg: float = 0.5,none_low_thresh: float = 0.35):
        """
        Parametros:
            ocr_model_num   Numero del modelo a usar (1-5)
        """
        if ocr_model_num not in MODELOS:
            raise KeyError('Modelo inexistente, valores posibles: (1-5)')
    ...
    def predict_ocr(self,x1: int,y1: int,x2: int,y2: int,frame: np.ndarray):
        ...
        alto,ancho,colors = frame.shape

        if alto < y2 or ancho < x2 or 0 > y1 or 0 > x1:
            return '',0
    ...
    def __probs_to_plate(self, prediction):
        prediction  = prediction.reshape((6, 36))
    ...
    def __predict_from_array(self, patente_recortada: np.ndarray):
        ...
        patente_recortada = cv2.resize(patente_recortada, dsize=(94, 24))
